...
def generate_book_embedding(book: models.Book, db: Session):
    ...
    embedding_record = db.query(models.BookEmbedding).filter(models.BookEmbedding.book_id == book.id).first()
    if embedding_record:
...
def semantic_search(query: str, db: Session, limit: int = 5) -> List[models.Book]:
    ...
    results = db.query(models.BookEmbedding).order_by(
        models.BookEmbedding.embedding.cosine_distance(query_vector)
    ).limit(limit).all()
    
    return [r.book for r in results if r.book]

def get_recommendations(user_id: int, db: Session, limit: int = 6) -> List[models.Book]:
    history = db.query(models.UserBrowsingHistory).filter(
        models.UserBrowsingHistory.user_id == user_id
    ).order_by(models.UserBrowsingHistory.timestamp.desc()).limit(3).all()
    ...
    embeddings = db.query(models.BookEmbedding).filter(models.BookEmbedding.book_id.in_(recent_book_ids)).all()
    ...
    results = db.query(models.BookEmbedding).filter(
        models.BookEmbedding.book_id.notin_(recent_book_ids)
    ).order_by(
        models.BookEmbedding.embedding.cosine_distance(avg_vector)
    ).limit(limit).all()
...
def process_chat(query: str, user_id: int, db: Session) -> str:
    ...
    response = (f"Based on your query, I found some great recommendations from our catalog! "
