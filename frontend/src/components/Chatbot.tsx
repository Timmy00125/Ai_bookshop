...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<{sender: 'bot' | 'user', text: string}[]>([
    { sender: 'bot', text: 'Hi there! I am your AI Bookshop Assistant. Looking for a recommendation or have a specific topic in mind? Let me help!' }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { sender: 'bot', text: 'Oops! I am having trouble connecting to my knowledge base right now. Please try again later.' }]);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed bottom-6 right-6 lg:bottom-10 lg:right-10 w-[380px] h-[550px] max-h-[80vh] bg-white rounded-3xl shadow-2xl shadow-indigo-900/10 border border-slate-100 flex flex-col z-50 overflow-hidden transform transition-all duration-300">
      ...
      <div className="bg-indigo-600 p-4 px-6 flex justify-between items-center text-white relative overflow-hidden flex-shrink-0">
        <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-2xl transform translate-x-1/2 -translate-y-1/2"></div>
         <div className="flex items-center gap-3 relative z-10">
            ...
            <div>
              <h3 className="font-extrabold text-lg tracking-tight">AI Assistant</h3>
              <p className="text-indigo-200 text-xs font-medium flex items-center gap-1"><Sparkles size={10} /> Online</p>
            </div>
         </div>
         <button onClick={() => setIsOpen(false)} className="text-indigo-200 hover:text-white p-2 bg-indigo-700/50 hover:bg-indigo-700 rounded-full transition relative z-10">
           <X size={20} />
         </button>
      </div>
      ...
      <div className="flex-grow overflow-y-auto p-5 pb-8 space-y-4 bg-slate-50 relative scroll-smooth">
        {messages.map((msg, idx) => (
          <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} group items-end gap-2`}>
             {msg.sender === 'bot' && (
