...
import api from '../api';
import { Trash2, ShoppingBag, ShoppingCart, Plus, Minus, ArrowRight, ShieldCheck } from 'lucide-react';
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-slate-500 text-lg mb-8 max-w-lg mx-auto">
          Thank you for your purchase. We've simulated the checkout process and your books will be delivered digitally (in theory!).
        </p>
        <div className="flex justify-center gap-4">
          <Link to="/orders" className="bg-indigo-600 text-white font-bold py-3 px-6 rounded-xl hover:bg-indigo-700 transition">
            View Orders
          </Link>
          <Link to="/" className="bg-white text-indigo-600 font-bold py-3 px-6 rounded-xl border border-indigo-200 hover:bg-indigo-50 transition">
            Continue Shopping
          </Link>
        </div>
      </div>
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-slate-500 mb-8 max-w-sm mx-auto">Looks like you haven't added any books to your cart yet. Discover your next read!</p>
        <Link to="/" className="bg-indigo-600 text-white font-bold py-3 pt-3.5 px-8 rounded-xl hover:bg-indigo-700 hover:shadow-lg transition inline-block">
          Explore Catalog
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
      <h1 className="text-3xl font-extrabold text-slate-900 mb-8 tracking-tight flex items-center gap-3">
        <ShoppingCart size={32} className="text-indigo-600" /> Your Shopping Cart
      </h1>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="lg:w-2/3 space-y-4">
          {cart.map(item => (
            <div key={item.id} className="bg-white p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col sm:flex-row gap-6 items-center group transition-all hover:border-slate-200">
              ...
              <div className="flex-grow flex flex-col w-full text-center sm:text-left">
                <div className="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-1">{item.book.genre}</div>
                <Link to={`/book/${item.book.id}`} className="font-bold text-lg text-slate-900 hover:text-indigo-600 transition truncate mb-1">
                  {item.book.title}
                </Link>
                <div className="text-slate-500 text-sm mb-4">{item.book.author}</div>
                
                <div className="mt-auto flex items-center justify-between">
                  <span className="font-extrabold text-xl text-slate-900">${(item.book.price * item.quantity).toFixed(2)}</span>
        ...
        <div className="lg:w-1/3">
          <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-6 sm:p-8 sticky top-24">
            <h3 className="text-xl font-extrabold text-slate-900 mb-6 tracking-tight">Order Summary</h3>
