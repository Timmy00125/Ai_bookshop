...
import { ArrowLeft, ShoppingCart, Star, Calendar, BookOpen, AlertCircle } from 'lucide-react';
  ...
  if (!book) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        ...
        <p className="text-slate-500 mb-6">We couldn't find the book you were looking for.</p>
          ...
          <div className="lg:w-2/3 flex flex-col">
            ...
            <div className="flex items-center gap-6 mb-8 border-b border-slate-100 pb-8">
              <div className="flex items-center gap-2 bg-amber-50 px-3 py-1.5 rounded-xl border border-amber-100">
                ...
                <span className="font-bold text-slate-800 text-lg">{book.average_rating}</span>
              </div>
              <div className="flex flex-col">
                <span className="font-bold text-3xl text-slate-900">${Number(book.price).toFixed(2)}</span>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-100 mt-auto">
              <div className="flex items-center gap-3">
                <div className="bg-white p-2 rounded-lg text-slate-400 shadow-sm border border-slate-100"><Calendar size={20} /></div>
                <div>
                  <div className="text-xs text-slate-400 font-bold uppercase tracking-wider">Published</div>
                  <div className="font-medium text-slate-800">{book.publication_date ? new Date(book.publication_date).toLocaleDateString() : 'N/A'}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="bg-white p-2 rounded-lg text-slate-400 shadow-sm border border-slate-100"><BookOpen size={20} /></div>
                <div>
                  <div className="text-xs text-slate-400 font-bold uppercase tracking-wider">Publisher</div>
                  <div className="font-medium text-slate-800">{book.publisher || 'N/A'}</div>
                </div>
              </div>
            </div>
          </div>
