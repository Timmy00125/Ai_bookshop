  ...
  if (orders.length === 0) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mb-2 tracking-tight">No Order History</h2>
        <p className="text-slate-500 mb-8 max-w-sm mx-auto">You haven't placed any simulated orders yet.</p>
        <Link to="/" className="bg-indigo-600 text-white font-bold py-3 pt-3.5 px-8 rounded-xl hover:bg-indigo-700 transition inline-block shadow-sm">
          Explore Catalog
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 lg:py-12">
      ...
      <div className="space-y-6">
        {orders.map(order => (
          <div key={order.id} className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden transform transition hover:shadow-md">
            <div className="bg-slate-50 border-b border-slate-200 p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex flex-wrap gap-6 text-sm">
                <div>
                  <span className="block text-slate-500 font-medium uppercase text-xs tracking-wider mb-1">Order Placed</span>
                ...
                <div>
                  <span className="block text-slate-500 font-medium uppercase text-xs tracking-wider mb-1">TotalAmount</span>
                  <span className="font-bold text-indigo-600 text-base">${Number(order.total_price).toFixed(2)}</span>
                </div>
                <div>
                  <span className="block text-slate-500 font-medium uppercase text-xs tracking-wider mb-1">Order Number</span>
                  ...
                  <div key={item.id} className="flex gap-4 sm:gap-6 items-center">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 sm:text-lg mb-1 line-clamp-2 hover:text-indigo-600 transition">
                        <Link to={`/book/${item.book_id}`}>{item.book.title}</Link>
                      </h4>
                      <p className="text-sm text-slate-500 mb-2">{item.book.author}</p>
